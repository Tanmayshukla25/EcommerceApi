...
function AddProductForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    // append image file
    if (imageFile) {
      payload.append("image", imageFile); // ✅ MUST MATCH backend
    }

    try {
      const res = await axios.post("http://localhost:4040/product/add", payload);
   ...
   <div className="min-h-screen bg-gradient-to-br mt-20 from-slate-50 via-blue-50 to-indigo-100 py-8 px-4 relative overflow-hidden">
      ...
      <form action="" onSubmit={handleSubmit}>
          ...
          <div className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 hover:from-blue-700 hover:via-blue-800 hover:to-indigo-800 text-white font-semibold px-6 py-4 rounded-xl transition-all duration-300 transform hover:scale-[1.02] hover:shadow-2xl focus:ring-4 focus:ring-blue-200 focus:outline-none shadow-lg active:scale-[0.98] relative overflow-hidden group"
            >
        ...
        <div className="text-center mt-6">
          <p className="text-gray-500 text-sm">All fields are required unless marked optional</p>
        </div>
      </div>
    </div>
  );
